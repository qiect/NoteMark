    ...
    public class OneMarkAddIn : IDTExtensibility2, IRibbonExtensibility
    {
        ...
        public void OnConnection(object application, ext_ConnectMode connectMode, object addInInst, ref Array custom)
        {
            ...
            try
            {
                ...
                logger.Info("NoteMarkAddIn: OnConnection called, mode=" + connectMode.ToString());
            ...
            {
                Debug.WriteLine("NoteMarkAddIn OnConnection error: " + ex.Message);
            }
        }
        ...
        public void OnDisconnection(ext_DisconnectMode removeMode, ref Array custom)
        {
            try
            {
                if (logger != null)
                {
                    logger.Info("NoteMarkAddIn: OnDisconnection called");
                }
                ...
                if (keyboardHook != null)
                {
                    ...
                    keyboardHook.ExitSourceModeRequested -= OnKeyboardExitSourceModeRequested;
                    keyboardHook.RealtimeRenderRequested -= OnKeyboardRealtimeRenderRequested;
                    keyboardHook.TabRenderRequested -= OnKeyboardTabRenderRequested;
            ...
            {
                Debug.WriteLine("NoteMarkAddIn OnDisconnection error: " + ex.Message);
            }
        }
        ...
        public void OnStartupComplete(ref Array custom)
        {
            try
            {
                if (logger != null)
                {
                    logger.Info("NoteMarkAddIn: OnStartupComplete called");
                }
                ...
                System.Threading.ThreadPool.QueueUserWorkItem(delegate
                {
                    try
                    {
                        ...
                        if (logger != null)
                        {
                            logger.Info("NoteMarkAddIn: deferred initialization complete");
                        }
                    }
                    catch (Exception ex)
                    {
                        if (logger != null)
                        {
                            logger.Error("NoteMarkAddIn: deferred initialization failed", ex);
                        }
                    }
                });
                ...
                if (logger != null)
                {
                    logger.Info("NoteMarkAddIn: OnStartupComplete finished");
                }
            }
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.Error("NoteMarkAddIn: OnStartupComplete failed", ex);
                }
                else
                {
                    Debug.WriteLine("NoteMarkAddIn OnStartupComplete error: " + ex.Message);
                }
            }
        }
        ...
        public void OnBeginShutdown(ref Array custom)
        {
            try
            {
                if (logger != null)
                {
                    logger.Info("NoteMarkAddIn: OnBeginShutdown called");
                }
            ...
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.Error("NoteMarkAddIn: OnBeginShutdown failed", ex);
                }
            }
        }
        ...
        public void OnAboutClick(IRibbonControl control)
        {
            try
            {
                ...
                var message = string.Format(
                    "NoteMark - OneNote Markdown 扩展\n版本: {0}\n\n在 OneNote 中渲染 Markdown 内容的扩展插件。",
                    version);
                MessageBox.Show(message, "关于 NoteMark", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            catch (Exception ex)
            {
                if (logger != null)
        ...
        private void OnKeyboardExitSourceModeRequested(object sender, EventArgs e)
        {
            try
            {
                if (renderHandler != null)
                {
                    renderHandler.HandleExitSourceMode();
                }
            }
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.Error("Keyboard ExitSourceModeRequested handler failed", ex);
                }
            }
        }

        private void OnKeyboardRealtimeRenderRequested(object sender, EventArgs e)
        {
            try
            {
                // Delay to let OneNote process the Enter key first (create new line, save content)
                ...
                timer = new System.Threading.Timer(delegate
                {
                    try
                    {
                        if (renderHandler != null)
                        {
                            renderHandler.HandleRealtimeRender();
                        }
                    }
                    catch (Exception ex)
                    {
                        if (logger != null)
                        {
                            logger.Error("Delayed RealtimeRender handler failed", ex);
                        }
                    }
                    finally
                    {
            ...
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.Error("Keyboard RealtimeRenderRequested handler failed", ex);
                }
            }
        }

        private void OnKeyboardTabRenderRequested(object sender, EventArgs e)
        {
            try
            {
                if (renderHandler != null)
            ...
            catch (Exception ex)
            {
                if (logger != null)
                {
                    logger.Error("Keyboard TabRenderRequested handler failed", ex);
                }
            }
        }
    }
